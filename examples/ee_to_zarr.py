...
_SCALE = flags.DEFINE_float(
    'scale', 0.25, help='Scale factor in degrees for output Zarr.'
)
...
def main(argv: list[str]) -> None:
  ...
  grid_params = helpers.fit_geometry(
      ...
      grid_scale=(
          _SCALE.value,
          -_SCALE.value,
      ),  # negative y-scale for north-up orientation
  )

  ds = xr.open_dataset(
      _INPUT.value, engine=xee.EarthEngineBackendEntrypoint, **grid_params
  )
